      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col lg:flex-row gap-12 items-start">
          <div className="lg:w-1/3">
            <div className="inline-flex items-center gap-2 bg-[#D51820]/10 rounded-full px-4 py-1.5 mb-6">
              <GraduationCap className="w-4 h-4 text-[#D51820]" /><span className="text-sm font-medium text-[#D51820]">Education Hub</span>
            </div>
            ...
            <button className="flex items-center gap-2 text-[#D51820] font-semibold hover:gap-3 transition-all">Explore All Resources <ArrowRight className="w-4 h-4" /></button>
          </div>
          <div className="lg:w-2/3 grid sm:grid-cols-2 gap-6">
            {resources.map((r, i) => (
              <div key={i} className="p-6 rounded-2xl border border-gray-100 bg-gray-50/50 hover:shadow-lg hover:border-[#D51820]/20 transition-all group cursor-pointer">
                <div className="w-12 h-12 bg-[#D51820]/10 rounded-xl flex items-center justify-center mb-4 group-hover:bg-[#D51820] transition-colors">
                  <r.icon className="w-6 h-6 text-[#D51820] group-hover:text-white transition-colors" />
                </div>
                <h3 className="text-lg font-semibold text-gray-900 mb-2">{r.title}</h3>
                <p className="text-sm text-gray-600 mb-3">{r.desc}</p>
                <span className="text-xs font-medium text-[#D51820]">{r.count}</span>
              </div>
            ))}
          </div>
        </div>
