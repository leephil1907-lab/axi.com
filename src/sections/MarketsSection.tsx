    ...
    <section id="markets" className="py-16 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8">
          ...
          <Link to={isAuthenticated ? '/trading' : '/signup'} className="inline-block bg-[#E5B80C] hover:bg-[#d4a90b] text-gray-900 px-8 py-3 rounded text-sm font-bold transition-colors">Power up your portfolio</Link>
        </div>
        <div className="flex flex-wrap justify-center gap-2 mb-8">
          {tabs.map(tab => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
              className={`px-6 py-2.5 rounded-full text-sm font-medium border transition-all ${activeTab === tab.key ? 'bg-[#D51820] text-white border-[#D51820]' : 'bg-white text-gray-700 border-gray-300 hover:border-gray-400'}`}>
              {tab.label}
            </button>
          ))}
        </div>
