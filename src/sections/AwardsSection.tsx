        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {awards.map((a, i) => (
            <div key={i} className="flex items-start gap-4 p-5 rounded-xl border border-gray-100 bg-gray-50/50 hover:shadow-md transition-all">
              <div className="w-12 h-12 bg-[#D51820]/10 rounded-lg flex items-center justify-center flex-shrink-0">
                <a.icon className="w-6 h-6 text-[#D51820]" />
              </div>
              <div><div className="text-xs font-medium text-[#D51820] mb-1">{a.year}</div><h3 className="text-base font-semibold text-gray-900">{a.title}</h3><p className="text-sm text-gray-500">{a.org}</p></div>
            </div>
          ))}
        </div>
      </div>
