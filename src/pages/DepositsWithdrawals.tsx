...
export default function DepositsWithdrawals() {
  ...
  return (
    <div className="min-h-screen bg-white">
      <Navbar />
      <div className="pt-14">
        <div className="bg-[#E5B80C] py-16 text-center">
        ...
        <div className="max-w-6xl mx-auto px-4 mb-6">
          <div className="flex justify-center gap-0 border-b border-gray-200">
            <button onClick={() => setTab('deposits')} className={`px-6 py-3 text-sm font-medium border-b-2 transition-colors ${tab==='deposits'?'border-[#D51820] text-[#D51820]':'border-transparent text-gray-500'}`}>Deposits</button>
            <button onClick={() => setTab('withdrawals')} className={`px-6 py-3 text-sm font-medium border-b-2 transition-colors ${tab==='withdrawals'?'border-[#D51820] text-[#D51820]':'border-transparent text-gray-500'}`}>Withdrawals</button>
          </div>
        </div>
        ...
        <div className="text-center mb-10"><Link to="/funds" className="inline-block bg-[#E5B80C] hover:bg-[#d4a90b] text-gray-900 px-8 py-3 rounded text-sm font-bold transition-colors">MAKE A DEPOSIT</Link></div>
        ...
        <div className="bg-gray-100 py-12 text-center">
          ...
          <div className="flex justify-center gap-3">
            <Link to="/funds" className="bg-gray-900 hover:bg-gray-800 text-white px-6 py-2.5 rounded text-xs font-bold transition-colors">TRY A FREE DEMO</Link>
            <Link to="/funds" className="bg-[#E5B80C] hover:bg-[#d4a90b] text-gray-900 px-6 py-2.5 rounded text-xs font-bold transition-colors">OPEN A LIVE ACCOUNT</Link>
          </div>
        </div>
      </div>
    </div>
