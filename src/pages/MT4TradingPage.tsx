...
import { 
  ...
  Shield, Zap, BarChart3, TrendingUp, 
...
const comparisonData = [
  { feature: "Spreads", demo: "Same as real", real: "From 0.0 pips" },
  { feature: "Leverage", demo: "Up to 1:500", real: "Up to 1:500" },
  { feature: "Instruments", demo: "All 1000+", real: "All 1000+" },
  { feature: "Execution", demo: "Instant", real: "Instant" },
  { feature: "Virtual Funds", demo: "$10,000 - $100,000", real: "Your deposit" },
  { feature: "Risk", demo: "None (virtual)", real: "Real money risk" },
  { feature: "Profit", demo: "Not withdrawable", real: "Fully withdrawable" },
  { feature: "EAs & Indicators", demo: "Full access", real: "Full access" },
];
          ...
          <div className="grid grid-cols-2 gap-12 items-center">
            <div>
              <div className="flex items-center gap-2 mb-4">
                <span className="bg-[#D51820] text-white text-xs px-3 py-1 rounded-full font-semibold">POPULAR</span>
              ...
              <div className="flex gap-4">
                <Link 
                  to="/register?type=demo&platform=mt4" 
                  className="flex-1 bg-white/10 border-2 border-white/30 hover:border-white/60 text-white py-4 px-6 rounded-xl text-center transition-all hover:bg-white/20"
                >
                  <div className="text-sm text-gray-400 mb-1">Practice Risk-Free</div>
                  <div className="text-lg font-bold">Open Demo Account</div>
                  <div className="text-xs text-gray-500 mt-1">$10,000 virtual funds</div>
                </Link>
                <Link 
                  to="/register?type=real&platform=mt4" 
                  className="flex-1 bg-[#D51820] hover:bg-red-700 text-white py-4 px-6 rounded-xl text-center transition-all shadow-lg shadow-red-900/30"
                >
                  <div className="text-sm text-white/80 mb-1">Start Trading Live</div>
                  <div className="text-lg font-bold">Open Real Account</div>
                  <div className="text-xs text-white/70 mt-1">Min deposit $100</div>
                </Link>
              </div>
            </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="h-32 bg-gray-900 rounded-lg flex items-end gap-1 p-2">
                    {[40, 60, 45, 70, 55, 80, 65, 90, 75, 85, 60, 95, 70, 88, 55, 78, 82, 65, 90, 72].map((h, i) => (
                      <div key={i} className="flex-1 bg-[#D51820] rounded-t" style={{ height: `${h}%`, opacity: 0.6 + (i * 0.02) }}></div>
                    ))}
                  </div>
          ...
          <div className="flex gap-8">
            {[
              ...
              <button
                ...
                className={`py-4 text-sm font-medium border-b-2 transition-colors ${activeTab === tab.key ? 'border-[#D51820] text-[#D51820]' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
              >
                {tab.label}
              </button>
            ))}
      ...
      {activeTab === 'overview' && (
        <div className="max-w-7xl mx-auto px-4 py-12">
          ...
          <div className="grid grid-cols-3 gap-6 mb-12">
            {mt4Features.map((feat, i) => (
              <div key={i} className="bg-white rounded-xl p-6 border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
                <feat.icon className="w-10 h-10 text-[#D51820] mb-4" />
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden mb-12">
            ...
            >
              <div>
                <h3 className="text-xl font-bold text-gray-900">Demo vs Real Account Comparison</h3>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-200">
                      ...
                      <th className="text-center py-3 text-gray-900 font-semibold bg-green-50">Real Account</th>
                    </tr>
                  </thead>
                  <tbody>
                    {comparisonData.map((row, i) => (
                      <tr key={i} className="border-b border-gray-100">
                        ...
                        <td className="py-3 text-center text-blue-600">{row.demo}</td>
          ...
          {/* CTA Section */}
          <div className="bg-gradient-to-r from-[#D51820] to-red-700 rounded-2xl p-8 text-white text-center">
            <h3 className="text-2xl font-bold mb-3">Ready to Start Trading?</h3>
            <p className="text-white/80 mb-6">Choose your account type and download MT4 in minutes</p>
            <div className="flex justify-center gap-4">
              <Link to="/register?type=demo&platform=mt4" className="bg-white text-[#D51820] px-8 py-3 rounded-lg font-semibold hover:bg-gray-100 transition-colors">
                Open Demo Account
              </Link>
              <Link to="/register?type=real&platform=mt4" className="bg-white/20 border-2 border-white text-white px-8 py-3 rounded-lg font-semibold hover:bg-white/30 transition-colors">
                Open Real Account
              </Link>
            </div>
          </div>
      ...
      {activeTab === 'download' && (
        <div className="max-w-7xl mx-auto px-4 py-12">
          ...
          <div className="flex justify-center gap-2 mb-8">
            {systemRequirements.map((sys) => (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${selectedOS === sys.os ? 'bg-[#D51820] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
              >
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-8 mb-8">
            {systemRequirements.filter(s => s.os === selectedOS).map(sys => (
              <div key={sys.os} className="text-center">
                <sys.icon className="w-16 h-16 text-[#D51820] mx-auto mb-4" />
                ...
                <button className="bg-[#D51820] text-white px-8 py-3 rounded-lg font-semibold hover:bg-red-700 transition-colors inline-flex items-center gap-2">
          ...
          <div className="bg-gray-900 rounded-xl p-8 text-white text-center">
            <Globe className="w-12 h-12 text-[#D51820] mx-auto mb-4" />
